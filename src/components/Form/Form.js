...
const Form = ({cart, total, clearCart, compraId}) => {
    ...
    const nombreIngresado = (event) => {setNombre(event.target.value)};
    //console.log(event.target.value) 
    const apellidoIngresado = (event) => {setApellido(event.target.value)};
    const direccionIngresada = (event) => {setDireccion(event.target.value)};
...
return (
    <div className={estilos.form}>
        <form action="" onSubmit={enviarSubmit}>
            <input type="text" placeholder='Nombre' name="nombre" value={nombre} onChange={nombreIngresado}/>
            <input type="text" placeholder='Apellido' name="apellido" value={apellido} onChange={apellidoIngresado}/>
            <input type="text" placeholder='Direccion' name="direccion" value={direccion} onChange={direccionIngresada}/>
